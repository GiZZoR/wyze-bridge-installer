...
class _Config:
    def __init__(self):
        self.APP_CONF = os.path.join(os.path.expanduser("~"), "wyze-bridge", "config", "app.env")
        self.APP_GUNICORN = False
        self.APP_IMG_DIR = os.path.join(os.path.expanduser("~"), "wyze-bridge", "img")
        self.APP_IP = "0.0.0.0"
        self.APP_PATH = os.path.join(os.path.expanduser("~"), "wyze-bridge", "app")
        self.APP_PORT = 5000
        self.APP_TOKENS_PATH = os.path.join(os.path.expanduser("~"), "wyze-bridge", "tokens")
        self.APP_USER = os.getlogin()
        self.APP_VERSION = "latest"
        self.MEDIA_MTX_VERSION = "latest"
        self.MEDIA_MTX_PATH = os.path.join(os.path.expanduser("~"), "wyze-bridge", "mediamtx")
        self.INSTALLATION_CONF = os.path.join(os.path.expanduser("~"), "wyze-bridge", "config", "install.json")
        self.read_config_file()

    def get_description(self, name):
        descriptions = {
            "APP_CONF": "Path to env file containing docker-wyze-bridge settings.",
            "APP_IMG_DIR": "Path to location where images will be stored.",           
            ...
            "APP_TOKENS_PATH": "Path where tokens will be stored",
    ...
    def write_config_file(self):
        if os.path.isdir(os.path.dirname(self.INSTALLATION_CONF)) == False:
            os.makedirs(os.path.dirname(self.INSTALLATION_CONF))
...
class _WyzeBridgeInstallation:
    ...
    def backup(self, output_path):
        ...
        for dir_name in [self.install_path]:
...
env $(cat {self._env_file} {scriptConfig.APP_CONF} | egrep -v "^(#|$)" | xargs) {CMD}
"""
        run_sh_path = os.path.join(os.path.expanduser("~"),"run_wyze_bridge.sh")
        if os.path.exists(run_sh_path):
            _print_color("run_wyze_bridge.sh already exists",_COLOR_RED)
        else:
            with open(run_sh_path, 'w') as run_sh_file:
                run_sh_file.write(run_sh_content)
            _FilesystemActions.chmod(run_sh_path, 0o770)
            _print_color(f"Created run script at: {run_sh_path}",_COLOR_PURPLE)

    def create_settings_file(self, file_path: str, img_dir: str):
        ...
        try:
            ...
            with open(file_path, 'w') as file:
                file.write(f"""# [OPTIONAL] Credentials can be set in the WebUI
...
IMG_DIR={img_dir}""")
    ...
    def install_ffmpeg(self):
        ...
        if ffmpeg.returncode == 1:
            ...
            _FilesystemActions.extract_tarball(tar_data=ffmpeg_latest_release,path=self.install_path,strip_dirs=4,file_name_contains="ffmpeg")
            ffmpeg = subprocess.run(os.path.join(self.install_path, "ffmpeg"), shell=True, capture_output=True)
    ...
    def patch_tokens_path(self, path):
        """Patches the path in wyzebridge config.py for storing tokens."""
        try:
            config_py_file = os.path.join(self.install_path, 'wyzebridge', 'config.py')
            if os.path.isfile(config_py_file):
                _print_color(f"Patching path for tokens in {config_py_file}", _COLOR_YELLOW)
                with open(config_py_file, 'r') as file:
                    data = file.read()
                data = data.replace('/tokens', path)
                with open(config_py_file, 'w') as file:
                    file.write(data)
        except Exception as e:
            _print_color(f"Error patching tutk IOTC path: {e}", _COLOR_RED)
            sys.exit(1)

    def patch_tutk_iotc_lib_path(self):
        """Patches the path in tutk.py to the IOTC lib."""
        try:
            tutk_py_file = os.path.join(self.install_path, 'wyzecam', 'tutk', 'tutk.py')
            if os.path.isfile(tutk_py_file):
                _print_color(f"Patching path to IOTC lib in {tutk_py_file}", _COLOR_YELLOW)
                with open(tutk_py_file, 'r') as file:
                    data = file.read()
                data = data.replace('/usr/local/lib/libIOTCAPIs_ALL.so', os.path.join(self.install_path, 'lib', 'lib.amd64'))
                with open(tutk_py_file, 'w') as file:
                    file.write(data)
        except Exception as e:
            _print_color(f"Error patching tutk IOTC path: {e}", _COLOR_RED)
            sys.exit(1)
...
def run_install():
    ...
    _FilesystemActions.create_application_folders(folders=[scriptConfig.APP_IMG_DIR, scriptConfig.APP_TOKENS_PATH, scriptConfig.APP_PATH], service_user=scriptConfig.APP_USER)
    ...
    wyze_bridge.create_settings_file(file_path=scriptConfig.APP_CONF, img_dir=scriptConfig.APP_IMG_DIR)
    ...
    wyze_bridge.patch_tokens_path(scriptConfig.APP_TOKENS_PATH)
    wyze_bridge.patch_tutk_iotc_lib_path()
    wyze_bridge.install_ffmpeg()
    wyze_bridge.create_run_sh()
